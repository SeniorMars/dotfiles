  ...
  \begin{columns}[T,onlytextwidth]
    \column{.48\textwidth}
      \begin{cookiecard}[title=Title-page wedge]
        \texttt{\textbackslash background\{image.jpg\}}
...
\begin{frame}{Build with LuaLaTeX}
  \cookiekicker{Command}
  \vspace{.55em}

  \begin{block}{Command line}
    \texttt{latexmk -lualatex starter.tex}
  \end{block}

  \begin{itemize}
    \item Run at least twice when section, subsection, bibliography, or total frame counts change.
    \item \texttt{latexmk} runs LuaLaTeX and Biber as many times as the document needs.
    \item The included CI workflow compiles the starter and the full demo.
  \end{itemize}
\end{frame}
